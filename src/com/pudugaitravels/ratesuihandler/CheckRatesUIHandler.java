...
public class CheckRatesUIHandler extends HttpServlet{

	private static final long serialVersionUID = 1L;
	final static Logger logger = LoggerFactory.getLogger(CheckRatesUIHandler.class);
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			logger.info("Running in CheckRatesUIHandler Get method");
			...
			if(session == null)
			{
				...
				logger.info("Session expired when user clicked on CheckToday's rate button in admin page ");
			...
			else
			{
				...
				RequestDispatcher rd = req.getRequestDispatcher("jsps/todays_rate.jsp");
				rd.forward(req, resp);
		...
		{
			logger.error("Exception in CheckRatesUIHandler",e);
